...
def main():
    ...
    trans_mat=[[ibysqrt2,0,0,0,bysqrt2],[0,ibysqrt2,0,bysqrt2,0,]\
	...
	cubic_harm=np.zeros((dim,dim),dtype=complex)
        tempmat=np.dot(np.dot(trans_mat.H,mat[0:d,0:d]),trans_mat)
	cubic_harm[0:d,0:d]=tempmat
	tempmat=np.dot(np.dot(trans_mat.H,mat[d:d+d,0:d]),trans_mat)
        cubic_harm[d:d+d,0:d]=tempmat
        tempmat=np.dot(np.dot(trans_mat.H,mat[0:d,d:d+d]),trans_mat)
        cubic_harm[0:d,d:d+d]=tempmat
        tempmat=np.dot(np.dot(trans_mat.H,mat[d:d+d,d:d+d]),trans_mat)
        cubic_harm[d:d+d,d:d+d]=tempmat

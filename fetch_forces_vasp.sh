...
forces_line=$(awk -v IGNORECASE=1 '/TOTAL-FORCE/{print NR}' OUTCAR)
...
awk -v awk_forces_line=$forces_line -v awk_no_atoms=$no_atoms 'NR==awk_forces_line+2,NR==awk_forces_line+2+awk_no_atoms {print $4 " " $5 " " $6}' OUTCAR > forces.dat

echo $no_atoms > no_atoms.dat

#convert_forces_to_atomic_units
#mv forces_au.dat forces.dat
